...
static void usage(void) {
  printf(
    ...
    "\n"
    " -i <str>   yamscan TSV output or a BED file (>=6 columns: seq, start,\n"
    "            end, name, score, strand). Can be gzipped. Use '-' for stdin.\n"
    "            yamscan TSV uses p-value to pick which overlap to keep; BED\n"
    "            uses the score column (lower discarded). Input must be sorted\n"
    "            by start within each seq/motif/strand combination (yamscan\n"
    "            output already is).\n"
