...
static void usage(void) {
  printf(
    ...
    "                                                                      \n"
    " -i <str>   Positives FASTA. Can be gzipped. Use '-' for stdin only  \n"
    "            when -n is also provided (cannot shuffle a stream).       \n"
    " -n <str>   Negatives FASTA. If omitted, positives are shuffled to   \n"
    "            generate the negative set (see -k and -s).               \n"
    " -m <str>   Motif file (MEME, JASPAR, HOMER, or HOCOMOCO PCM).       \n"
    " -o <str>   Output TSV file. Default: stdout.                        \n"
    " -b <dbl,   Comma-separated background probabilities for A,C,G,T|U.  \n"
    "     dbl,   Defaults to MEME file background or uniform 0.25.        \n"
    "     dbl,                                                             \n"
    "     dbl>                                                             \n"
    " -p <int>   Pseudocount for PWM generation. Default: %d.             \n"
    " -N <int>   Number of motif sites for PPM->PCM conversion.           \n"
    "            Default: %d.                                             \n"
    " -d         Deduplicate motif/sequence names (default: abort on      \n"
    "            duplicates). Duplicates will have index appended.        \n"
    " -r         Do not trim motif (HOCOMOCO/JASPAR only) and sequence    \n"
    "            names to the first word.                                 \n"
    ...
    " -k <int>   Shuffle k-mer order when -n is absent. Default: %d.     \n"
    "            k=1 uses Fisher-Yates; k>=2 uses Eulerian (exact k-mer). \n"
